...
describe("FileIO", function() {
    ...
    beforeEach(function (done) {
        mkdirp(path.join(testDir, "dir1"), function () {
            fs.writeFile(path.join(testDir, "dir1", "file1"), "This is a test file.", done)
        });
    });
    ...
    it("returns an error from an asynchronous read of a nonexistant file", function () {
        return fileIO.read(path.join(testDir, "dir2", "file1"))
            .then((data) => {
    ...
    it("overwrites an existing file asynchronously", function () {
        return fileIO.write(path.join(testDir, "dir1", "file1"), "Different text.")
    ...
    it("writes a new file asynchronously", function () {
        return fileIO.write(path.join(testDir, "dir1", "file2"), "Some more text.")
    ...
    it("deletes a file asynchronously", function () {
        return fileIO.delete(path.join(testDir, "dir1", "file1"))
    ...
    it("deletes a directory asynchronously", function () {
        return fileIO.delete(path.join(testDir, "dir1"))
    ...
    it("returns an error trying to asynchronously delete a file that does not exist", function () {
        return fileIO.delete(path.join(testDir, "dir1", "file2"))
            .then(() => {
