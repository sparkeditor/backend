...
describe("Auth", function() {
    ...
    beforeEach(function() {
        return new Promise(function(resolve, reject) {
            ...
            .then(() => {
                return database.insertInto("project", [
                    { name: "project1" },
                    { name: "project2" }
                ]);
            });
    ...
    it("hashes identical passwords with different salts", function() {
        return auth
            .addUser({ username: "user1", password: "password" })
            .then(() =>
                auth.addUser({ username: "user2", password: "password" })
            )
            .then(() => database.query("user", { id: 1 }))
            .then(rows => rows[0])
            .then(user1 => {
    ...
    it("retrieves a user's access level for a project", function() {
        return auth
            .addUser({
                username: "user1",
                password: "password",
                projects: [{ 1: auth.accessLevels.ADMIN }]
            })
    ...
    it("changes a user's access level to a project", function() {
        return auth
            .addUser({
                username: "user1",
                password: "password",
                projects: [{ 1: auth.accessLevels.CONTRIBUTOR }]
            })
            .then(() => auth.setAccess("user1", 1, auth.accessLevels.ADMIN))
            .then(() => auth.getAccess("user1", 1))
            .then(accessLevel => {
        ...
        return auth
            ...
            .catch(err => {
                ...
                expect(err.message).to.equal(
                    "user1 does not have access to project id 1"
                );
            });
    });
