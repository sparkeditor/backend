...
describe("Database", function() {
  ...
  it("creates a new table", function() {
    return database
      .createTable("test1", {
        foo: { type: "integer", primaryKey: true },
        bar: { type: "text" }
      })
      .then(() =>
        db.getAsync(
          "SELECT sql, count(*) FROM sqlite_master WHERE type='table' AND name='test1'"
        )
      )
      .then(result => {
        expect(result["count(*)"]).to.equal(1);
        expect(result.sql.toLowerCase()).to.equal(
          "create table test1 (foo integer primary key, bar text)"
        );
      });
  });
  ...
  it("fails silently creating a table that already exists", function() {
    return database
      ...
      .then(() =>
        database.createTable("test1", {
          differentFoo: { type: "integer", primaryKey: true },
          differentBar: { type: "text" }
        })
      )
      .then(() =>
        db.getAsync(
    ...
    return database
      ...
      .then(() =>
        database.createTable("test2", {
          ...
          test1_fk: { type: "integer", foreignKey: { test1: "foo" } }
        })
      )
      .then(() => database.insertInto("test1", { foo: 3, bar: "test" }))
      .then(() => database.insertInto("test2", { id: 1, test1_fk: 3 }))
      .then(() =>
        database.query({ test1: "foo", test2: "test1_fk" }, { bar: "test" })
      )
      .then(result => {
        expect(result).to.be.an("array");
        expect(result).to.have.length(1);
