...
const projectDirectory = configurationManager.getValue(
  "projectDirectory",
  path.join(os.homedir(), ".spark", "projects")
);
...
module.exports = {
  ...
  createProject: function(projectDefinition) {
    ...
    if (rootDirectory) {
      return fs
        .accessAsync(rootDirectory)
        .then(() => database.insertInto("project", projectDefinition))
        .then(() => mkdirpAsync(projectDirectory))
        .then(() =>
          fs.symlinkAsync(
            rootDirectory,
            path.join(projectDirectory, projectDefinition.name)
          )
        );
    } else {
      return database
        .insertInto("project", projectDefinition)
        .then(() =>
          mkdirpAsync(path.join(projectDirectory, projectDefinition.name))
        );
    }
  },
  /**
  ...
  getProject: function(project) {
    ...
    return database.query("project", queryDefinition).then(rows => {
      if (rows.length === 0) {
        throw doesNotExistError(queryDefinition.project);
      }
  ...
  /**
     ...
     * @returns {Promise} - Resolves with either an array of user objects, with an added access_level
     *                      property denoting their access level with that project, or an error
     */
  getUsersForProject: function(projectId) {
    return database
      .query({ user: "id", user_project: "user_id" }, { project_id: projectId })
      .then(rows => {
  ...
  getProjectsForUser: function(user) {
    if (typeof user === "string") {
      ...
      return database
        .query({ project: "id", user_project: "project_id" }, { user_id: user })
        .then(rows => {
          rows.forEach(function(row) {
            delete row.user_id;
            delete row.project_id;
      ...
      return Promise.reject(
        new Error("user parameter must be a string or a number")
      );
    }
  },
  /**
      ...
      .then(() => {
        ...
        if (filepath.includes(projectDirectory)) {
          ...
          const projectName = filepath
            .replace(projectDirectory + path.sep, "")
            .split(path.sep)[0];
          ...
          return fs
            .readdirAsync(projectDirectory)
            .filter(function(projectDir) {
              projectDir = path.join(projectDirectory, projectDir);
              return fs.lstatAsync(projectDir).then(stats => {
                if (stats.isSymbolicLink()) {
                  return fs
                    .readlinkAsync(projectDir)
                    .then(linkTarget => filepath.includes(linkTarget));
                } else {
                  return false;
                }
              });
            ...
            .then(matchingProjects => {
              if (matchingProjects.length > 1) {
                throw new Error(
                  "Found more than one matching project based on symlink paths"
                );
              }
              if (matchingProjects[0]) {
                return self
                  .getProject(matchingProjects[0])
