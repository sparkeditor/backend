...
module.exports = {
  ...
  addUser: function(userDefinition) {
    return bcrypt
      .hash(userDefinition.password, SALT_ROUNDS)
      .then(hash => {
        return {
      ...
      .then(() => {
        if (userDefinition.projects) {
          return database
            .query("user", { username: userDefinition.username })
            .then(rows => {
  ...
  removeUser: function(username) {
    return database.query("user", { username: username }).then(rows => {
      const user = rows[0];
      if (!user) {
        throw doesNotExistError(username);
      }
      return database.deleteRow("user", { username: username }).then(() => {
  ...
  setAccess: function(username, projectId, accessLevel) {
    return database.query("user", { username: username }).then(rows => {
      ...
      return database
        .deleteRow("user_project", { user_id: user.id, project_id: projectId })
        .then(() => {
          return database.insertInto("user_project", {
            user_id: user.id,
            project_id: projectId,
            access_level: accessLevel
          });
  ...
  getAccess: function(username, projectId) {
    return database
      .query(
        { user: "id", user_project: "user_id" },
        { username: username, project_id: projectId }
      )
      .then(rows => {
        if (rows.length === 0) {
          throw new Error(
            username + " does not have access to project id " + projectId
          );
        }
  ...
  revokeAccess: function(username, projectId) {
    return database.query("user", { username: username }).then(rows => {
      ...
      return database.deleteRow("user_project", {
        user_id: user.id,
        project_id: projectId
      });
    });
    ...
    return database.query("user", { username: username }).then(rows => {
      ...
      return bcrypt.compare(password, user.password).then(matches => {
        if (matches && projectId) {
          return database
            .query("user_project", { user_id: user.id, project_id: projectId })
            .then(rows => {
