...
module.exports = {
                ...
                if (userDefinition.projects) {
                    return database
                        ...
                        .then(rows => {
                            ...
                            const projects = [];
                            userDefinition.projects.forEach(function(
                                projectAccessLevel
                            ) {
    ...
    removeUser: function(username) {
        return database.query("user", { username: username }).then(rows => {
            ...
            return database
                .deleteRow("user", { username: username })
                .then(() => {
        ...
        return database
            ...
            .then(rows => {
                if (rows.length === 0) {
                    throw new Error(
                        username +
                            " does not have access to project id " +
                            projectId
                    );
                }
