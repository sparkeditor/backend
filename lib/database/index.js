...
const database = {
    ...
    setDB: function(newDBPath) {
        return fileIO.createDir(path.dirname(newDBPath))
            .then(function () {
        ...
        for (const columnName in definition) {
            ...
            if (columnDef.foreignKey) {
                for (const foreignTableName in columnDef.foreignKey) {
                    if (!columnDef.foreignKey.hasOwnProperty(foreignTableName)) {
                        continue;
                    }
                    foreignColumnName = columnDef.foreignKey[foreignTableName];
                    constraintClause += "FOREIGN KEY (" + columnName + ") REFERENCES " + foreignTableName + "(" + foreignColumnName + "), ";
                }
            }
            sql = sql.slice(0, -1);
            sql += ", ";
    ...
    query: function(tableName, queryDefinition) {
        ...
        if (joinClause) {
            ...
            sql += "JOIN " + joinTableName + " ON " + joinTableName + "." + joinTableColumn + " = " + tableName + "." + tableColumn + " ";
        }
