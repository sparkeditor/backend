                ...
                if (!hasAccess) {
                    ...
                    if (!serverContext.hasFile(file)) {
                        return fileIO.read(file)
                            .then((contents) => {
                                serverContext.openFile(file, new PieceTable(contents));
                                serverContext.addClient(file, client.id);
                                client.broadcast.emit("open", {file: file, username: serverContext.getUsername(client.id)});
                                return {status: constants.status.OKAY, contents: contents, clients: null};
                            })
                            .catch((err) => {
                                if (err.code === "ENOENT") {
                        ...
                        client.broadcast.emit("open", {file: file, username: serverContext.getUsername(client.id)});
                        return {status: constants.status.OKAY, contents: serverContext.getFileContents(file), clients: clients};
                    }
                }
            })
