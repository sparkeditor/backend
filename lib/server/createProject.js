        ...
        auth.authenticate(credentials.username, credentials.password)
            .then((hasAccess) => {
                if (!hasAccess) {
                    ...
                    return projectManager.createProject({name: projectName})
                        .then(() => projectManager.getProject(projectName))
                        .then((project) => auth.setAccess(credentials.username, project.id, auth.accessLevels.ADMIN)
                            .then(() => {
                                return {status: constants.status.OKAY, project: project};
                            })
                        );
                }
            })
