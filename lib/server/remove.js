...
module.exports = function(client, serverContext) {
   return function(data, callback) {
       ...
       projectManager.getProjectForFile(file)
           .then((projectId) => [auth.authenticate(credentials.username, credentials.password, projectId), auth.getAccess(credentials.username, projectId)])
           ...
           .then((hasAccess) => {
               if (!hasAccess) {
                   ...
                   return fileIO.delete(file).then(() => {
